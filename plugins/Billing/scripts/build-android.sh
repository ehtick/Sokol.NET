#!/bin/bash
# Build libsokol_billing.so for Android — standalone Billing plugin library.
# Output: libs/android/<abi>/{debug,release}/libsokol_billing.so
...
# Usage:    ./plugins/Billing/scripts/build-android.sh
#
...
echo "========================================="
echo "sokol_billing — Android build"
...
for ABI in "${ANDROID_ABIS[@]}"; do
    ...
    BUILD_DIR="$REPO_ROOT/build-sokol-billing-android-$ABI"
    rm -rf "$BUILD_DIR"
    mkdir -p "$BUILD_DIR"
    ...
    echo "  -> plugins/Billing/libs/android/$ABI/release/libsokol_billing.so"
done

echo ""
echo "========================================="
echo "sokol_billing — Android done!"
for ABI in "${ANDROID_ABIS[@]}"; do
    echo "  $ABI: plugins/Billing/libs/android/$ABI/release/libsokol_billing.so"
done
echo "========================================="
echo ""
echo "Next: rebuild the APK so libsokol_billing.so is packaged alongside libsokol.so."
