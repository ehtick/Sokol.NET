// 8×8 bitmap font for the score card renderer.
// Covers printable ASCII 0x20–0x7E (95 glyphs × 8 bytes = 760 bytes).
...
public static class GlyphAtlas
{
    ...
    // Draw text centred at pixel (cx, cy) on an RGBA canvas of size canvasW × canvasH.
    // scaleX / scaleY magnify each pixel (e.g. 3 → 24 px tall glyphs).
    public static void DrawCentered(
        byte[] px, int canvasW, int canvasH,
        string text,
        int scaleX, int scaleY,
        int cx, int cy,
        (byte r, byte g, byte b) color)
    {
        int totalW = text.Length * GlyphW * scaleX;
        int totalH = GlyphH * scaleY;
        int startX = cx - totalW / 2;
        int startY = cy - totalH / 2;

        for (int ci = 0; ci < text.Length; ci++)
        {
            int gx = startX + ci * GlyphW * scaleX;
            BlitGlyph(px, canvasW, canvasH, text[ci], gx, startY, scaleX, scaleY, color);
        }
    }

    static void BlitGlyph(
        byte[] px, int canvasW, int canvasH,
        char c, int x0, int y0,
        int scaleX, int scaleY,
        (byte r, byte g, byte b) color)
    {
        int idx = c - 0x20;
        if (idx < 0 || idx >= 95) return;  // unsupported character → skip
        int glyphBase = idx * GlyphH;

        for (int row = 0; row < GlyphH; row++)
        {
            byte bits = s_font[glyphBase + row];
            for (int col = 0; col < GlyphW; col++)
            {
                if ((bits & (0x80 >> col)) == 0) continue;  // transparent pixel

                int px0 = x0 + col * scaleX;
                int py0 = y0 + row * scaleY;
                ...
                for (int dx = 0; dx < scaleX; dx++)
                {
                    int px1 = px0 + dx;
                    int py1 = py0 + dy;
                    if ((uint)px1 >= (uint)canvasW || (uint)py1 >= (uint)canvasH) continue;
                    int i = (py1 * canvasW + px1) * 4;
                    px[i]   = color.r;
                    px[i+1] = color.g;
                    px[i+2] = color.b;
                    px[i+3] = 255;
    ...
    // ── 8×8 font data — printable ASCII 0x20–0x7E ──────────────────────────────
    // One glyph = 8 bytes (rows top→bottom, MSB = left pixel).
    static readonly byte[] s_font =
    {
        ...
        /* 0x26 '&' */ 0x38,0x6C,0x68,0x76,0xDC,0xCE,0x7B,0x00,
        ...
        /* 0x30 '0' */ 0x38,0x6C,0x6E,0x7E,0x76,0x6C,0x38,0x00,
        ...
        /* 0x4E 'N' */ 0x63,0x73,0x7B,0x6F,0x67,0x63,0x63,0x00,
        ...
        /* 0x56 'V' */ 0x66,0x66,0x66,0x66,0x3C,0x3C,0x18,0x00,
        ...
        /* 0x66 'f' */ 0x1C,0x30,0x30,0x78,0x30,0x30,0x30,0x00,
