...
public abstract class ShapeFilter : NativeObject
{
    ...
    private static unsafe byte ShouldCollideCallback(nint context, nint shape2, SubShapeID* subShapeIDOfShape2)
    {
        // TODO: Add cache for Shape
        ShapeFilter listener = DelegateProxies.GetUserData<ShapeFilter>(context, out _);
        return (byte)(listener.ShouldCollide(Shape.GetObject(shape2)!, *subShapeIDOfShape2) ? 1 : 0);
    }

    [UnmanagedCallersOnly]
    private static unsafe byte ShouldCollide2Callback(nint context, nint shape1, SubShapeID* subShapeIDOfShape1, nint shape2, SubShapeID* subShapeIDOfShape2)
    {
        // TODO: Add cache for Shape
        ShapeFilter listener = DelegateProxies.GetUserData<ShapeFilter>(context, out _);
        return (byte)(listener.ShouldCollide(Shape.GetObject(shape1)!, *subShapeIDOfShape1, Shape.GetObject(shape2)!, *subShapeIDOfShape2) ? 1 : 0);
    }
}
