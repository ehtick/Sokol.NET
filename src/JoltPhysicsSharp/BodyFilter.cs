...
public abstract class BodyFilter : NativeObject
{
    ...
    private static byte ShouldCollideCallback(nint context, uint bodyID)
    {
        BodyFilter listener = DelegateProxies.GetUserData<BodyFilter>(context, out _);
        return (byte)(listener.ShouldCollide(new BodyID(bodyID)) ? 1 : 0);
    }

    [UnmanagedCallersOnly]
    private static byte ShouldCollideLockedCallback(nint context, nint body)
    {
        BodyFilter listener = DelegateProxies.GetUserData<BodyFilter>(context, out _);
        return (byte)(listener.ShouldCollideLocked(Body.GetObject(body)!) ? 1 : 0);
    }
}
