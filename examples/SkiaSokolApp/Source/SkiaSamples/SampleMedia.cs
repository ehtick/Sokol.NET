...
public static class SampleMedia
{
	...
	public static class Embedded
	{
		...
		public static Stream? Load(string name)
		{
			...
			var resourceName = resources.FirstOrDefault(n => n == name);
			
			// If not found, try with .Media. prefix
			if (resourceName == null)
			{
				var searchName = $".Media.{name}";
				resourceName = resources.FirstOrDefault(n => n.EndsWith(searchName));
			}
			...
			if (stream == null)
			{
				Console.WriteLine($"Failed to load resource stream: {resourceName}");
			}
			return stream;
		}
	}
