...
/// <summary>
/// Port of Samples/Tests/Rig/LoadSaveRigTest.cpp.
/// Loads Human.tof, adds a distance constraint between the wrist joints,
/// and creates a dynamic ragdoll.
/// </summary>
class Demo_LoadSaveRig : DemoBase
{
    public override string Name     => "Load Save Rig";
    ...
    public override unsafe void Init(JPH.BodyInterface bi, JPH.PhysicsSystem sys, List<PhysicsBody> bodies, Random random)
    {
        ...
        {
            var dc = new JPH.DistanceConstraintSettings();
            dc.mMaxDistance = 0.1f;
            dc.mMinDistance = 0.1f;
            // Space = LocalToBodyCOM (byte value 0)
           dc.SetSpace(0);
