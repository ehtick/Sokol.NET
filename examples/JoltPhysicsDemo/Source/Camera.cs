...
namespace Sokol
{
    ...
    public class Camera
    {
        ...
        public bool SuppressArrowKeys { get; set; } = false;
        ...
        {
            if (desc.Aspect <= 0f) desc.Aspect = 60f;
            if (desc.NearZ  <= 0f) desc.NearZ  = 0.1f;
            if (desc.FarZ   <= 0f) desc.FarZ   = 1000f;
            _desc = desc;
        ...
        // Resets touch tracking state. Call when a touch is intercepted by virtual
        // controls so stale _touch1IsActive cannot cause spurious camera orbiting.
        ...
        public unsafe void HandleEvent(sapp_event* ev)
        {
            ...
            switch (ev->type)
            {
                case sapp_event_type.SAPP_EVENTTYPE_MOUSE_DOWN:
                    if (ev->mouse_button == sapp_mousebutton.SAPP_MOUSEBUTTON_MIDDLE)
                    {
                        _middleMouseDown = true;
                        sapp_lock_mouse(true);
                    }
                    else if (ev->mouse_button == sapp_mousebutton.SAPP_MOUSEBUTTON_LEFT)
                    {
        ...
        private unsafe void HandleKeyboardInput(sapp_event* ev)
        {
            ...
            switch (ev->key_code)
            {
                ...
                case sapp_keycode.SAPP_KEYCODE_UP:
                    if (!SuppressArrowKeys) _keyUp = isDown;
                    break;
                case sapp_keycode.SAPP_KEYCODE_DOWN:
                    if (!SuppressArrowKeys) _keyDown = isDown;
                    break;
