...
public static unsafe class Shaders 
{
    /*
        ...
        Cmdline:
            sokol-shdc --input shaders/instancing-compute-sapp.glsl --output shaders/compiled/ios/instancing-compute-sapp-shader.cs --slang metal_ios -f sokol_csharp
...
[StructLayout(LayoutKind.Sequential)]
    ...
    */
    static readonly byte[] cs_init_source_metal_ios = {
    ...
    */
    static readonly byte[] cs_update_source_metal_ios = {
    ...
    */
    ...
    public static unsafe sg_shader_desc init_shader_desc(sg_backend backend) {
        if (backend == SG_BACKEND_METAL_IOS) {
            sg_shader_desc desc = default(sg_shader_desc);
            desc.compute_func.source = System.Text.Encoding.UTF8.GetString(cs_init_source_metal_ios);
    ...
    public static unsafe sg_shader_desc update_shader_desc(sg_backend backend) {
        if (backend == SG_BACKEND_METAL_IOS) {
            sg_shader_desc desc = default(sg_shader_desc);
            desc.compute_func.source = System.Text.Encoding.UTF8.GetString(cs_update_source_metal_ios);
            desc.compute_func.entry = "main0";
            desc.mtl_threads_per_threadgroup.x = 64;
            desc.mtl_threads_per_threadgroup.y = 1;
            desc.mtl_threads_per_threadgroup.z = 1;
