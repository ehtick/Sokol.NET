...
# Interactive iOS Install Script (Debug)
# Usage: interactive-ios-install-debug.sh <project_path>
...
CACHE_DIR="$HOME/.Sokol.NET-cache"
...
echo "🔍 Checking for iOS devices and simulators (Debug mode)..."
...
if [ "$total_devices" -eq 0 ]; then
    ...
    echo "🚀 Building and installing (Debug mode)..."

    if [ "$device_category" = "physical" ]; then
        dotnet msbuild "$PROJECT_PATH" -t:BuildIOSDebugInstall -p:IOSDeviceId="$device" -p:IOSDevelopmentTeam="$development_team"
    else
        dotnet msbuild "$PROJECT_PATH" -t:BuildIOSSimulatorDebug -p:IOSSimulatorId="$device" -p:IOSDevelopmentTeam="$development_team"
    fi
else
    ...
    if [ -t 0 ] && [ -t 1 ]; then
        ...
        while true; do
            ...
            if [[ "$selection" =~ ^[0-9]+$ ]] && [ "$selection" -ge 1 ] && [ "$selection" -le "$total_devices" ]; then
                ...
                if [ "$device_category" = "physical" ]; then
                    dotnet msbuild "$PROJECT_PATH" -t:BuildIOSDebugInstall -p:IOSDeviceId="$selected_device" -p:IOSDevelopmentTeam="$development_team"
                else
                    dotnet msbuild "$PROJECT_PATH" -t:BuildIOSSimulatorDebug -p:IOSSimulatorId="$selected_device" -p:IOSDevelopmentTeam="$development_team"
                fi
                break
            else
        ...
        if command -v osascript >/dev/null 2>&1; then
            ...
            selection=$(osascript -e "tell app \"System Events\" to display dialog \"Select iOS Device/Simulator (Debug):\" & return & \"$device_list\" default answer \"1\" with title \"iOS Device Selection (Debug)\"" -e "text returned of result" 2>/dev/null)
