...
# MSVC names the DLL as assimp-vc143-mt.dll (or similar with different versions)
# Search for any assimp*.dll file
$DllPath = Get-ChildItem -Path "$BuildDir\bin\$BuildType" -Filter "assimp*.dll" -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty FullName

if (-not $DllPath) {
    # Try alternative location
    $DllPath = Get-ChildItem -Path "$BuildDir\$BuildType\bin" -Filter "assimp*.dll" -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty FullName
}

if ($DllPath -and (Test-Path $DllPath)) {
    Copy-Item "$DllPath" "$DestDir\assimp.dll"
    Write-Host "  Copied: $(Split-Path $DllPath -Leaf) -> assimp.dll" -ForegroundColor Green
    
    # Also copy lib file if it exists
    $LibDir = Split-Path $DllPath -Parent | Split-Path -Parent | Join-Path -ChildPath "lib\$BuildType"
    $LibPath = Get-ChildItem -Path "$LibDir" -Filter "assimp*.lib" -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty FullName
    if ($LibPath -and (Test-Path $LibPath)) {
        Copy-Item "$LibPath" "$DestDir\assimp.lib"
        Write-Host "  Copied: $(Split-Path $LibPath -Leaf) -> assimp.lib" -ForegroundColor Green
    }
} else {
