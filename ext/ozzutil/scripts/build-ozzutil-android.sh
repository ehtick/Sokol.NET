...
if [ ! -d "$OZZ_ANIMATION_DIR/bin/android/$ABI/$(echo "$BUILD_TYPE" | tr '[:upper:]' '[:lower:]')" ]; then
    ...
    if [ -f "$OZZ_ANIMATION_DIR/scripts/build-ozz-animation-android.sh" ]; then
        "$OZZ_ANIMATION_DIR/scripts/build-ozz-animation-android.sh" "$ABI" "$BUILD_TYPE"
    else
        echo "Error: ozz-animation build script not found at $OZZ_ANIMATION_DIR/scripts/build-ozz-animation-android.sh"
        exit 1
    fi
fi
