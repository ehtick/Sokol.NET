...
if [ ! -d "$OZZ_ANIMATION_DIR/bin/emscripten/wasm32/$(echo "$BUILD_TYPE" | tr '[:upper:]' '[:lower:]')" ]; then
    ...
    if [ -f "$OZZ_ANIMATION_DIR/scripts/build-ozz-animation-web.sh" ]; then
        "$OZZ_ANIMATION_DIR/scripts/build-ozz-animation-web.sh" "$BUILD_TYPE"
    else
        echo "Error: ozz-animation build script not found at $OZZ_ANIMATION_DIR/scripts/build-ozz-animation-web.sh"
        exit 1
    fi
fi
