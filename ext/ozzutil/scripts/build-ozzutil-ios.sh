...
if [ ! -d "$OZZ_ANIMATION_DIR/bin/ios/$ARCH/$(echo "$BUILD_TYPE" | tr '[:upper:]' '[:lower:]')" ]; then
    ...
    if [ -f "$OZZ_ANIMATION_DIR/scripts/build-ozz-animation-ios.sh" ]; then
        "$OZZ_ANIMATION_DIR/scripts/build-ozz-animation-ios.sh" "$ARCH" "$BUILD_TYPE"
    else
        echo "Error: ozz-animation build script not found at $OZZ_ANIMATION_DIR/scripts/build-ozz-animation-ios.sh"
        exit 1
    fi
fi
